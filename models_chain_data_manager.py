...
class models_chain_dataset(Dataset):
    def __init__(self, root, image_size, goto_pixel,
                 use_transforms=False, overfit_mode=False, use_advanced_transforms:bool=True):
        print(f'[I (models_chain_dataset)] - root={root}\n'
        ...
        self.image_size_rgb2d = (self.image_size[0] * 2, self.image_size[1] * 2)
        print("self.image_size", self.image_size)
        print("self.image_size_rgb2d", self.image_size_rgb2d)
    ...
    def transform(self, rgb_rgb2d, rgb, depth):
        # Resize to constant spatial dimensions
        rgb_rgb2d = T.Resize(self.image_size_rgb2d)(rgb_rgb2d)
        ...
        rgb_rgb2d = T.ToTensor()(rgb_rgb2d)
        rgb       = T.ToTensor()(rgb)
        depth     = T.ToTensor()(depth)
        
        # Dynamic range [0,1] -> [-1, 1]
        rgb_rgb2d = T.Normalize(mean=(.5,.5,.5), std=(.5,.5,.5))(rgb_rgb2d)
        rgb       = T.Normalize(mean=(.5,.5,.5), std=(.5,.5,.5))(rgb)
        depth     = T.Normalize(mean=(.5,), std=(.5,))(depth)
        
        return rgb_rgb2d, rgb, depth
    ...
    def __getitem__(self, index):
        ...
        if self.use_transforms:
            rgb_rgb2d, rgb, d = self.transform(rgb_rgb2d, rgb, d)
...
def models_chain_dataloader(root, batch_size, num_workers, train_test_ratio, image_size,
                              use_transforms=False, overfit_mode=False, seed=42, inference=None, goto_pixel=True):
    print(f'[I (models_chain_dataloader)] - root={root}\n'
...
#                                                overfit_mode=overfit_mode, goto_pixel=goto_pixel)
        rgbd_grads_ds = models_chain_dataset(root, image_size, use_transforms=use_transforms,
                                               overfit_mode=overfit_mode, goto_pixel=goto_pixel)
...
#         rgbd_grads_ds = rgbd_gradients_inference_dataset(root, image_size, inference=inference, goto_pixel=goto_pixel)
    ...
    if not overfit_mode:
        ...
        print(f'[I (models_chain_dataloader)] - |Train Dataset|={len(ds_train)}, |Test Dataset|={len(ds_test)}')
        ...
        dl_overfit = torch.utils.data.DataLoader(rgbd_grads_ds,
                                                 ...
                                                 num_workers=num_workers,
                                                 shuffle=True)
        print(f'[I (models_chain_dataloader)] - |Dataset|={len(rgbd_grads_ds)}')
