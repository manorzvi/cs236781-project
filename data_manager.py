...
class rgbd_gradients_dataset(Dataset):
    def __init__(self, root, image_size, constant_index=None, use_transforms=False, overfit_mode=False):
        ...
        self.depth_inference_path = os.path.join(root, 'depth_inference/0.png')
...
#         rgb, depth = RotateAndFillCornersWithImageFrameColors(output_size=IMAGE_SIZE, degrees_range=20, frame_removal_length=3)(rgb, depth)
    ...
    def __getitem__(self, index):
        ...
        d           = Image.open(d_path)
        d_inference = Image.open(self.depth_inference_path)

        if self.use_transforms:
            rgb, d = self.transform(rgb, d)
            d_inference = T.Resize(self.image_size)(d_inference)
            d_inference = T.ToTensor()(d_inference)
            d_inference = T.Normalize(mean=(.5,), std=(.5,))(d_inference)

        x,y = calc_grads(d)

        return {'rgb': rgb,
                'depth': d,
                'depth_inference': d_inference,
                'x'    : x,
                'y'    : y}
